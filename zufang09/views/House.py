...
class IndexHandler(BaseHandler):
    ...
    def get(self):
        ...
        if ret:
            json_houses = ret
            print("redis_json_houses= ", json_houses)
            # print(json.loads(json_houses))
            json_houses = str(json_houses, 'UTF-8')
            ...
            try:
                ...
                house_ret = self.db.query("select hi_house_id,hi_title,hi_order_count,hi_index_image_url from ih_house_info " \
                                          "order by hi_house_id desc limit %s;" % constants.HOME_PAGE_MAX_HOUSES)
                print('house_ret= ', house_ret)
            except Exception as e:
                logging.error(e)
            ...
            for l in house_ret:
                ...
                house = {
                    ...
                    "title":l["hi_title"],
                    "img_url": '/static/images/uploadImage/' + l["hi_index_image_url"].split('/')[-1]
                }
                houses.append(house)
            print('houses= ', houses)
            json_houses = json.dumps(houses)
            print("mysql_json_houses= ", json_houses)
            # json_houses = str(json_houses, 'UTF-8')
            # print("json_houses= ", json_houses)
            try:
        ...
        if ret:
            json_areas = ret
            print("json_areas= ", json_areas)
            json_areas = str(json_areas, 'UTF-8')
            print("json_areas= ", json_areas)
        else:
            try:
...
class AreaInfoHandler(BaseHandler):
    def get(self):
        ...
        if ret:
            ...
            print("ret=", ret)
            ret = str(ret, encoding='utf-8')    # 把二进制转一下
            print("ret=", ret)
            resp = '{"errcode":"0", "errmsg":"OK", "data":%s}' % ret
            print('hit redis: area_info')
...
class MyHousesHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ret = self.db.query("select a.hi_house_id,a.hi_title,a.hi_price,a.hi_ctime,b.ai_name,"
                                "a.hi_index_image_url from ih_house_info a inner join ih_area_info b"
                                " on a.hi_area_id=b.ai_area_id where a.hi_user_id=%s;", userId)
        except Exception as e:
            logging.error(e)
            self.write({'errcode': RET.DBERR, 'errmsg': 'get data err'})
        houses = []
        if ret:
            for item in ret:
                house = {
                    ...
                    'title': item['hi_title'],
                    'price': item['hi_price'],
                    'ctime': item['hi_ctime'].strftime('%Y-%m-%d'),
                    'area_name': item['ai_name'],
                    'img_url': config.image_url_prefix + item['hi_index_image_url'] if item['hi_index_image_url'] else ''
                }
                houses.append(house)
...
class HouseInfoHandler(BaseHandler):
    def get(self):
        ...
        if ret:
            for image in ret:
                images.append('/static/images/uploadImage/' + image["hi_url"].split('/')[-1])
        data["images"] = images
    ...
    def post(self):
        ...
        try:
            ...
            try:
                ...
                return self.write(dict(errcode=RET.DBERR, errmsg='no date save'))
...
class HouseImageHandler(BaseHandler):
    ...
    def post(self):
        ...
        img_name = self.storage(house_image)
        if not img_name:
        ...
        try:
            ...
            sql = "insert into ih_house_image(hi_house_id,hi_url) values(%(house_id)s,%(img_name)s)"
            sql1 = "update ih_house_info set hi_index_image_url=%(img_name)s " \
                  "where hi_house_id=%(house_id)s and hi_index_image_url is null"
            self.db.execute(sql, house_id=house_id, img_name=img_name)
            self.db.execute(sql1, house_id=house_id, img_name=img_name)
            # print(sql)
            # self.db.execute(sql)
        except Exception as e:
            logging.error(e)
        ...
        img_url = '/static/images/uploadImage/' + img_name.split('/')[-1]
        self.write({"errcode":RET.OK, "errmsg":"OK", "url":img_url})

    def storage(self, house_image):
        print(house_image)
        img = cv2.imdecode(np.frombuffer(house_image, np.uint8), cv2.IMREAD_COLOR)
        rgb_img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        imageAll = os.listdir(config.settings['static_path'] + '/images/uploadImage/')
        maxName = max(imageAll)
        iamgeName = int(maxName.split('.')[0]) + 1
        # print(config.settings['static_path']+'/images/uploadImage/1.jpg')
        cv2.imwrite(config.settings['static_path'] + '/images/uploadImage/%04d.jpg' % iamgeName, rgb_img)
        # print('rgb_img= ', rgb_img)
        # a = Image.fromarray(rgb_img)
        # a.show()
        print('存储图片')
        return config.settings['static_path'] + '/images/uploadImage/%04d.jpg' % iamgeName
        ...
        if ret:
            for l in ret:
                house = dict(
                    ...
                    image_url='/static/images/uploadImage/'+l["hi_index_image_url"].split('/')[-1] if l.get("hi_index_image_url") else ""
                )
                data.append(house)
