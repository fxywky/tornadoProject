...
class OrderHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            # 保存订单数据ih_order_info，
            print('保存订单数据ih_order_info')
            self.db.execute("insert into ih_order_info(oi_user_id,oi_house_id,oi_begin_date,oi_end_date,oi_days,oi_house_price,oi_amount) values(%(user_id)s,%(house_id)s,%(begin_date)s,%(end_date)s,%(days)s,%(price)s,%(amount)s);",
                            user_id=user_id, house_id=house_id, begin_date=start_date, end_date=end_date, days=days, price=house["hi_price"], amount=amount)
            self.db.execute("update ih_house_info set hi_order_count=hi_order_count+1 where hi_house_id=%(house_id)s;", house_id=house_id)
            print('保存订单数据ih_order_info完成')
        except Exception as e:
            logging.error(e)
        ...
        if ret:
            for l in ret:
                order = {
                    ...
                    "title":l["hi_title"],
                    "img_url":'/static/images/uploadImage/' + l["hi_index_image_url"].split('/')[-1] if l["hi_index_image_url"] else "",
